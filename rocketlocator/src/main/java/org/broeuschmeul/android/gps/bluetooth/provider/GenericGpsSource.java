...
public abstract class GenericGpsSource extends Observable {
    ...
    *
    * @param nmeaSentence the complete NMEA sentence received from the bluetooth GPS (i.e. $....*XY
    ...
    protected void notifyNmeaSentence(final String nmeaSentence) {
        ...
        try {
            ...
            SharedHolder.getInstance()
                    .getLogs()
                    .e(LOG_TAG, "error while parsing NMEA sentence: " + nmeaSentence, e);
            nmeaValues = null;
            Sounds.gps_disconnected.start();
        }

        if (nmeaValues != null
                && nmeaValues.getCommand() != null
                && nmeaValues.getCommand().equals("GPGGA")) {
